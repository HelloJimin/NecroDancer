...
item * itemManager::addItem(string name,int x, int y)
{
	...
	if (name == "가죽갑옷")return new body("가죽갑옷",BODY,1,"방어력 1의 갑옷이다",x,y,50);
	if (name == "쇠갑옷")return new body("쇠갑옷",BODY,2,"방어력 2의 갑옷이다",x,y,200);
	if (name == "도복")return new body("도복",BODY,3,"공격력2배이다",x,y,90);
